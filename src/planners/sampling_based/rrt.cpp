...
#include "search/planners/sampling_based/rrt.hpp"

ims::RRT::RRT(const ims::RRTParams& params) : params_(params), SamplingBasedPlanner(params){}

void ims::RRT::initializePlanner(const std::shared_ptr<ActionSpace>& action_space_ptr,
                                   const StateType& start,
                                   const StateType& goal) {
    rrt_action_space_ptr_ = std::dynamic_pointer_cast<RRTActionSpace>(action_space_ptr);
    action_space_ptr_ = rrt_action_space_ptr_;
    // Clear both.
    rrt_action_space_ptr_->resetPlanningData();
    resetPlanningData();

    // check if start is valid
    if (!rrt_action_space_ptr_->isStateValid(start)){
        throw std::runtime_error("Start state is not valid");
    }
    // check if goal is valid
    if (!rrt_action_space_ptr_->isStateValid(goal)){
        throw std::runtime_error("Goal state is not valid");
    }
    int start_ind_ = rrt_action_space_ptr_->getOrCreateRobotState(start);
    auto search_start_ = getOrCreateSearchState(start_ind_);
    search_start_->g = 0.0;

    int goal_ind_ = rrt_action_space_ptr_->getOrCreateRobotState(goal);
...
auto ims::RRT::getSearchState(int state_id) -> ims::RRT::SearchState * {
    assert(state_id < states_.size() && state_id >= 0);
    return states_[state_id];
}

auto ims::RRT::getOrCreateSearchState(int state_id) -> ims::RRT::SearchState * {
...
void ims::RRT::sampleRoboState(StateType& sampled_robo_state) {
    StateType sampled_state;
    rrt_action_space_ptr_->sampleState(sampled_state);
    sampled_robo_state = sampled_state;
}

// Must make sure we are not extending from the goal state here.
void ims::RRT::getNearestSearchState(const StateType& sample_state, SearchState*& nearest_state, double& nearest_state_distance) {
    ...
    for (auto& state : states_) {
        ...
        StateType state_from_tree = rrt_action_space_ptr_->getRobotState(state->state_id)->state;
...
double ims::RRT::distanceRoboStates(const StateType& state_1, const StateType& state_2) {
    return ims::euclideanDistance(state_1, state_2);
}

double ims::RRT::distanceSearchState(SearchState*& state_1, SearchState*& state_2) {
    StateType rob_state_1 = rrt_action_space_ptr_->getRobotState(state_1->state_id)->state;
    StateType rob_state_2 = rrt_action_space_ptr_->getRobotState(state_2->state_id)->state;
    return distanceRoboStates(rob_state_1, rob_state_2);
}

void ims::RRT::resetPlanningData(){
...
void ims::RRT::reconstructPath(std::vector<StateType>& path) {
...
void ims::RRT::reconstructPath(std::vector<StateType>& path, std::vector<double>& costs) {
    path.clear();
    costs.clear();
...
bool ims::RRT::plan(std::vector<StateType> &path){
    startTimer();
    for (int i=0; i < params_.max_itr_; i++){
        ...
        if(canReachGoal(new_search_state)) {
            ...
            stats_.num_generated = (int)rrt_action_space_ptr_->states_.size();
            return true;
        }
    }
    return false;
...
bool ims::RRT::extend(SearchState*& state_to_extend, const StateType& sampled_robo_state, SearchState*& new_search_state) {
    int cur_state_id = state_to_extend->state_id;
    StateType succ_state = StateType(sampled_robo_state.size());
    rrt_action_space_ptr_->getSuccessorInDirection(cur_state_id, sampled_robo_state, succ_state, params_.step_size_);
    // If trapped (no movement at all), return false
    if (succ_state.empty()) {
        return false;
    }
    if (succ_state == rrt_action_space_ptr_->getRobotState(cur_state_id)->state) {
        return false;
    }
    // If reach
    if (succ_state == sampled_robo_state) {
        int new_state_id = rrt_action_space_ptr_->getOrCreateRobotState(succ_state);
...
bool ims::RRT::canReachGoal(SearchState*& cur_search_state) {
    // success
    int cur_state_id = cur_search_state->state_id;
    StateType goal_state = rrt_action_space_ptr_->getRobotState(goal_)->state;
    StateType succ_state = StateType(goal_state.size());
    rrt_action_space_ptr_->getSuccessorInDirection(cur_state_id, goal_state, succ_state, params_.step_size_);

    if (rrt_action_space_ptr_->isSameState(goal_state, succ_state)) {
        return true;
    }
    return false;
}
