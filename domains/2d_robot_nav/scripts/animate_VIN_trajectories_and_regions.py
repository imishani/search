...
def animate_vin_regions_and_trajs(map_ind, scaler, map_type):
    """
    Animate the paths in the order they are provided
    :param scaler:
    :param map_ind: the index of the map to use
    :param map_type: Type of the map. Either 'octile' or 'gridworld'.
    :return:
    """
    if map_type == 'octile':
        # load the map (octile map)
    ...
    # Load the trajectories from the file vin_trajectories.txt
    with open('/home/itamar/work/code/algorithms/search/domains/2d_robot_nav/scripts/vin_trajectories.txt', 'r') as f:
        # Each line is a state, and each state is a comma-separated pair of coordinates
        # When there is an empty line, it is the end of a trajectory
        lines = f.readlines()
        trajectories = []
        trajectory = []
        for line in lines:
...
if __name__ == "__main__":
    ...
    # visualize the paths
    if args.animate:
        animate_vin_regions_and_trajs(map_index, scale, args.map_type)
